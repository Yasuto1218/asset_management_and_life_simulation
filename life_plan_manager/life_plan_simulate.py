...
class LifePlanSimulate():
    ...
    def plot_figure_of_life_plan(self, all_property, age_list, all_life_simulate_list):
        ...
        ax1.bar(age_list, all_life_simulate_list, alpha=0.3, color='red')
        plt.ticklabel_format(style='plain',axis='y')
        plt.title(f'資産額:{all_property:,}円({self.simulate_age}歳時点)', fontsize=14)
        plt.grid()
        plt.yticks(np.arange(0, all_life_simulate_list[0] * 1.1, 2000000))
        plt.show()
    ...
    def plot_figure_of_life_plan_in_invest(self, age_list, all_life_simulate_list, invest_life_plan_simulate_list):
        fig = plt.figure(figsize=(10, 7))
        ax1 = fig.add_subplot(1, 1, 1)
        ax1.bar(age_list, invest_life_plan_simulate_list, alpha=0.9, color='blue')
        ax1.bar(age_list, all_life_simulate_list, alpha=0.9, color='red')
        plt.ticklabel_format(style='plain',axis='y')
        plt.title(f'投資ありのライフプラン({self.age}から{self.simulate_age}歳)', fontsize=14)
        plt.grid()
        plt.yticks(np.arange(0, invest_life_plan_simulate_list[-1] * 1.1, 5000000))
        plt.show()
